...
import {
  updateNpcWithCustomGifImage,
  updateNpcWithGameImage,
} from "../update-npc-image";
    ...
    const callback = function (newNpc) {
      if (newNpc.d && newNpc.d.id === npc.id) {
        ...
        if (img.src.endsWith("gif")) {
          if (
            img.src.startsWith("https://micc.garmory-cdn.cloud/obrazki/npc/")
          ) {
            const icon = img.src.substring(43);
            updateNpcWithGameImage(newNpc, icon);
          } else if (
            img.src.startsWith("https://micc.garmory-cdn.cloud/obrazki/")
          ) {
            const aNpath = window.CFG.r_npath;
            const regex =
              /^https:\/\/micc\.garmory-cdn\.cloud\/obrazki\/(.+?)\//;
