...
test("title : lvl*8 above lvl names lenght", function () {
  ...
  expect(nerthus.tips.title(PLAYER)).to.be.equal(
    nerthus.lvlNames[nerthus.lvlNames.length - 1],
  );
});
...
test("other : nick + clan", function () {
  ...
  expect(nerthus.tips.other(other)).equal(
    "<b>" + other.nick + "</b>" + "[" + other.clan.name + "]<br>",
  );
});
...
test("other : nick + mute", function () {
  ...
  expect(nerthus.tips.other(other)).equal(
    "<b>" + other.nick + "</b>" + "<img src=img/mute.gif>",
  );
});

test("other : nick + title", function () {
  var _title = nerthus.tips.title;
  nerthus.tips.title = function () {
    return "VIP";
  };

  var other = { nick: "NICK" };
  expect(nerthus.tips.other(other)).equal("<b>" + other.nick + "</b>" + "VIP");
  nerthus.tips.title = _title;
});

test("other : nick + rank", function () {
  var _rank = nerthus.tips.rank;
  nerthus.tips.rank = function () {
    return "RANK";
  };

  var other = { nick: "NICK" };
  expect(nerthus.tips.other(other)).equal(
    "<b>" + other.nick + "</b>" + "<i>RANK</i>",
  );
...
test("nerthus.tips.npc : nick + type=1 + wt>99 (tytan)", function () {
  ...
  expect(nerthus.tips.npc(npc)).equal(
    "<b>" + npc.nick + "</b>" + "<i>" + "tytan" + "</i>",
  );
});
...
test("nerthus.tips.npc : nick + type=2|3 + lvl diff <= 9 (Zwykły przeciwnik)", function () {
  ...
  hero.lvl = 0;
  expect(nerthus.tips.npc(npc)).equal(
    "<b>" + npc.nick + "</b>" + "<span >" + "Zwykły przeciwnik" + "</span>",
  );
});
...
test("nerthus.tips.npc : type=1 in group", function () {
  ...
  hero.lvl = 0;
  expect(nerthus.tips.npc(npc)).equal(
    "<b>" + npc.nick + "</b>" + "<span >" + ", w grupie" + "</span>",
  );
});
