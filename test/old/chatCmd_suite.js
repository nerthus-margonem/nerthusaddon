...
test("pseudo random generates same number with same seed", () => {
  ...
  expect(nerthus.chatCmd.cards.pseudoRandom(number)).to.be.equal(
...
test("pseudo random generates different numbers with different seed", () => {
  ...
  expect(nerthus.chatCmd.cards.pseudoRandom(number)).to.not.be.equal(
    nerthus.chatCmd.cards.pseudoRandom(number2),
  );
});
...
test("getCard returns 52 cards proper cards in deck type 52 (depended on pseudorandom)", () => {
  ...
  for (let i = 0; i < deck_type; i++) {
    const card = nerthus.chatCmd.cards.getCard(
      deck_id,
      Math.random(),
      deck_type,
    );
