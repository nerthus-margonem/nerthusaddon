...
before(function () {
  ...
  $._data = function () {
    return { keydown: [{ handler: function () {} }] };
  };
  $.LOADED_SCRIPTS = [];
  $.LOADED_JSONS = [];
  $.getJSON = function (url, callback) {
    this.LOADED_JSONS.push(url);
    callback({ version: VERSION_CURRENT });
  };

  $.getScript = function (url, callback) {
    nerthus.scripts = [];
    callback();
  }; //hack for loading NN_start, otherwise exception is raised due to lack of loaded NN_dlaRadnych which define nerthus.scripts
  require("../../NN_start.js");
...
beforeEach(function () {
  ...
  nerthus.RUNABLE_MODULE_ONE.start = function () {
    this.RUNNING = "SI";
  };
  nerthus.RUNABLE_MODULE_ONE.start_ni = function () {
    this.RUNNING = "NI";
  };
  nerthus.RUNABLE_MODULE_TWO = { RUNNING: false };
  nerthus.RUNABLE_MODULE_TWO.start = function () {
    this.RUNNING = "SI";
  };
  nerthus.RUNABLE_MODULE_TWO.start_ni = function () {
    this.RUNNING = "NI";
  };
...
test("GitHubLoader", function () {
  ...
  expect($.LOADED_SCRIPTS[0]).to.be.equal(
    nerthus.addon.fileUrl("NN_dlaRadnych.js"),
  );
  expect($.LOADED_SCRIPTS[1]).to.be.equal(nerthus.addon.fileUrl("NN_base.js"));
  expect($.LOADED_SCRIPTS[2]).to.be.equal(
    nerthus.addon.fileUrl("NN_worldEdit.js"),
  );
  expect($.LOADED_SCRIPTS[3]).to.be.equal(
    nerthus.addon.fileUrl(ADDITIONAL_SCRIPTS[0]),
  );
  expect($.LOADED_SCRIPTS[4]).to.be.equal(
    nerthus.addon.fileUrl(ADDITIONAL_SCRIPTS[1]),
  );
});
...
test("run addon in new interface", function () {
  //TODO find a way to test if runAddon correctly gets cookie
...
test("SI: other modules are started even if start of previous failed", function () {
  nerthus.RUNABLE_MODULE_ONE.start = function () {
    throw new Error("module one start error");
  };
